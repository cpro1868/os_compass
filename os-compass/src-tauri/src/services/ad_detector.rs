...
fn is_url_in_whitelist_internal(url: &str) -> bool {
    let result = (|| {
...
pub async fn mark_as_ad_internal(
    ...
    if is_url_in_whitelist_internal(source_url) {
    ...
    if judgment.is_ad {
        ...
        for keyword in &judgment.ad_keywords {
            if let Ok(pattern_id) = add_ad_pattern_internal(
                "keyword",
                keyword,
