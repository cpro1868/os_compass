...
fn url_in_whitelist_internal(url: &str) -> bool {
    let result = (|| {
...
pub fn check_ad_pattern(
    ...
    let mut stmt = conn
        .prepare("SELECT id, pattern_type, pattern_value, confidence FROM ad_patterns WHERE enabled = 1 AND confidence >= 50")
        .map_err(|e| e.to_string())?;
...
#[command]
pub fn init_ad_patterns() -> Result<(), String> {
    let db_guard = SYSTEM_DB.lock().map_err(|e| e.to_string())?;
    let db = db_guard.as_ref().ok_or("System DB not initialized")?;
    let conn = db.get_connection();
