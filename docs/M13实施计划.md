...
**目标**：实现二期 M13 功能：系统插件管理基础设施、信息源引擎、情报雷达插件（RSS/网站/TG 监控 + 收件箱）、意图搜索插件（ChatGPT 对话 UI）。

> **设计说明**：意图搜索的向量搜索增强功能在 **M17** 中实现（M17: 首页重构 + 向量搜索）。

**架构**：三层架构——底层是插件管理基础设施（FeaturePlugin trait + PluginRegistry + PluginManager + plugin_config_db），中层是信息源引擎（SourceAdapter trait + 四种适配器 + LLM 解析流水线），上层是两个功能插件实现（RadarPlugin + SearchPlugin）。

**操作域层次**（详见 `docs/插件接入规范.md` §8.5）：

| 域 | 存储位置 | 说明 |
|----|---------|------|
| 插件域 | `${AppData}/.os-compass/plugins.db` | 系统级插件配置（enabled、config） |
| 仓库域 | 当前仓库 `os_compass.db` | LLM/代理配置（运行时注入到 PluginContext） |
| 业务域 | `{vault_dir}/plugin_*.db` | 插件业务数据（订阅源、扫描结果等） |

**技术栈**：Rust + Tauri 2.0 + React 18 + TypeScript 5 + Tailwind CSS 3 + SQLite + Tokio + feed-rs + reqwest
- 前端: 无额外依赖（后端处理）

## 全局约束

- **二期功能标记**：所有代码和文档必须标注为二期（Phase 2）功能
- **Rust 编译检查**：`cargo check --lib` 验证编译（因 Tauri GUI 依赖无法在 CLI 运行）
...
- **插件域**：`${AppData}/.os-compass/plugins.db` 独立存储，不依赖主库
- **仓库域**：PluginContext 运行时注入，来自当前仓库配置
- **业务域**：vault 模式插件数据在 `{vault_dir}/plugin_*.db`，跟随仓库
- **仓库切换**：插件域不变，业务域跟随仓库切换重新加载

- **插件数据库策略**：四种模式（main/global/vault/none），详见 `docs/插件接入规范.md` §2
...
| `src-tauri/src/plugin_config_db.rs` | 独立插件配置库（plugins.db） |
...
| `src-tauri/src/plugins/search.rs` | SearchPlugin 实现（FeaturePlugin trait + 双轨搜索） |
...
| 文件 | 修改内容 |
|------|----------|
| `src-tauri/src/lib.rs` | 注册新模块 + 启动时初始化 PluginManager + 注册新命令 + get_vault_dir |
| `src-tauri/src/commands/mod.rs` | 添加 plugin_cmd/radar_cmd/search_cmd 模块声明 |
| `src-tauri/src/plugins/mod.rs` | 添加 radar/search 模块声明 |
| `src-tauri/src/db.rs` | 添加 plugin_get_db_path 函数 |
| `src-tauri/Cargo.toml` | 添加 feed-rs + async-trait + rusqlite + serde_json 依赖 |
...
| `src/components/SettingsDialog.tsx` | 新增插件管理标签页（PluginCard + PluginConfigModal） |
| `src/locales/zh.json` | 新增 radar/search/pluginConfig/storage/pluginLevel 命名空间 |
...
Task 3 ──> Task 5 (SourceAdapter) ──> Task 6 (LLM Parser) ──> Task 8A (Embedding) ──> Task 8 (SearchPlugin) ──> Task 10 (SearchCmd) ──> Task 12 (SearchView UI)
Task 4 ──> Task 13 (前端集成 + Sidebar + Settings)
Task 11 + Task 12 ──> Task 13
Task 13 ──> Task 14 (i18n + 暗色模式 + 构建)
Task 11 ──> Task 18 (雷达搜索增强)
```
...
## Task 8: SearchPlugin 完整实现（向量搜索 + LLM 联网）

**Files:**
- Modify: `os-compass/src-tauri/src/plugins/search.rs`（替换占位实现）
- Create: `os-compass/src-tauri/src/commands/search_cmd.rs`
- Create: `os-compass/src-tauri/src/embedding.rs`（向量生成和搜索）

**Interfaces:**
- Consumes: FeaturePlugin trait, sqlite-vec, LLM, db
- Produces: SearchPlugin 完整实现 + search_sources/search_history 表 + 搜索 Tauri 命令

### 8.1 搜索设计

> **说明**：M13 意图搜索使用关键词匹配作为基础实现，向量搜索增强在 **M17** 中实现。

**搜索流程**（M13）：
```
用户输入需求 → LLM 提取关键词 → 关键词 LIKE 匹配本地项目库
```

**Step 1: 实现搜索插件独立库表初始化**

在 search.rs 中添加 `init_search_db(vault_dir)` 函数：打开 plugin_search.db，创建 search_sources/search_history 两张表，创建索引。在 FeaturePlugin::init 中调用。

**Step 2: 实现 SearchPlugin 核心逻辑**

`intent_search(query, context)` 函数：
1. 调用 LLM 提取关键词（构建 prompt，调用 crate::llm，解析返回的关键词列表）
2. 在主库 projects 表中按关键词 LIKE 搜索
3. 合并结果，按 stars 排序
4. 保存搜索历史
...
git commit -m "feat(M13): implement SearchPlugin with keyword search"
```
...
**目标**：对已采集数据进行搜索、筛选、分页展示，增强用户体验。

**参照原型**：`design-system/public/radar-inbox.html`（已更新）
...
| 分页 | 每页数量可选（10/20/50），完整分页导航 |

### 与采集筛选的区别

| 场景 | 筛选方式 | 时机 |
|------|---------|------|
| 采集筛选 | 信息源级别（启用/禁用哪些信息源） | 采集时，控制采集哪些源 |
| **搜索增强** | **对已采集数据的二次筛选** | **采集后，按条件查看数据** |

### 后端改动

**Files:**
- Modify: `src-tauri/src/commands/radar_cmd.rs`

**`get_radar_items` 命令新增参数**：

```rust
pub fn get_radar_items(
    status: Option<String>,
    limit: Option<i64>,
    keyword: Option<String>,       // 新增：关键字搜索
    source_ids: Option<Vec<i64>>,  // 新增：信息源 ID 列表
    start_date: Option<String>,    // 新增：开始日期 (YYYY-MM-DD)
    end_date: Option<String>,      // 新增：结束日期 (YYYY-MM-DD)
    page: Option<i64>,            // 新增：页码（从 1 开始）
    page_size: Option<i64>,       // 新增：每页数量
) -> Result<PaginatedRadarItems, String>

// 返回结构
struct PaginatedRadarItems {
    items: Vec<RadarItem>,
    total: i64,       // 总数
    page: i64,        // 当前页
    page_size: i64,   // 每页数量
...
WHERE 1=1
  AND (? IS NULL OR status = ?)                           -- status 筛选
  AND (? IS NULL OR project_name LIKE ? OR description LIKE ?)  -- keyword 筛选
  AND (? IS NULL OR source_id IN (?))                    -- source_ids 筛选
  AND (? IS NOT NULL AND fetched_at >= ?)                -- start_date 筛选
  AND (? IS NOT NULL AND fetched_at <= datetime(?, '+1 day'))  -- end_date 筛选
...
**Files:**
- Modify: `os-compass/src/components/RadarInbox.tsx`
- Modify: `os-compass/src/api/radar.ts`
...
1. **顶部栏**：
   ...
   - 保留清空按钮
   - 保留立即扫描按钮
   - 新增切换信息源面板按钮（显示/隐藏右侧面板）

2. **搜索增强工具栏**（tabs 区域下方）：
   - 关键字搜索输入框（回车触发）
   - 信息源多选下拉（全选/取消/应用/清除）
   - 日期范围选择器（开始日期 ~ 结束日期）
   - 快捷日期按钮（今天/近7天/近30天/全部）
   - 重置筛选按钮（筛选条件存在时显示）
   - 筛选条件标签（可单独清除）

3. **底部栏**：
   - 总数统计 + 每页数量选择
   - 完整分页导航（首页/末页/上一页/下一页）
