...
impl SourceAdapter for GitLabAdapter {
    ...
    async fn fetch_project(&self, url: &str) -> Result<ProjectMetadata, SourceError>;
}
```
...
pub struct PluginMarket {
    local_plugins: Vec<LocalPlugin>,
    remote_plugins: Vec<RemotePlugin>,
...
| 命令 | 参数 | 返回值 |
|------|------|--------|
| `list_market_plugins` | - | `Vec<RemotePlugin>` |
| `install_plugin` | plugin_id | `Result<(), String>` |
