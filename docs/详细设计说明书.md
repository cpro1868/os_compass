...
| prompt_template_id | number | 使用的 Prompt 模板 ID（可选） |
...
**表：`article_prompt_templates`**

| 字段 | 类型 | 说明 |
|------|------|------|
| id | INTEGER PRIMARY KEY | 模板 ID |
| name | VARCHAR(100) | 模板名称 |
| style | VARCHAR(50) | 关联风格 |
| prompt | TEXT | Prompt 内容 |
...
| 风格 | 模板名称 | Prompt 模板 |
|------|----------|-------------|
| 技术科普 | 技术科普默认 | 你是一位技术科普作家。请根据项目信息写一篇通俗易懂的文章... |
| 商业推广 | 商业推广默认 | 你是一位商业文案专家。请写一篇突出项目商业价值的推广文章... |
| 技术博客 | 技术博客默认 | 你是一位资深开发者。请写一篇深度技术博客... |
| 简介说明 | 简介说明默认 | 请简洁明了地介绍这个项目... |
| 幽默吐槽 | 幽默吐槽默认 | 请用轻松幽默的语气介绍这个项目... |
...
**示例 Prompt（技术博客）**：
```
你是一位资深开发者。请根据以下信息写一篇技术博客风格的介绍文章。
...
// 获取所有模板
#[tauri::command]
async fn get_article_prompt_templates() -> Result<Vec<PromptTemplate>, String>;

// 创建模板
#[tauri::command]
async fn create_article_prompt_template(
    name: String,
    style: String,
    prompt: String,
) -> Result<i64, String>;
...
// 更新模板
#[tauri::command]
async fn update_article_prompt_template(
    id: i64,
    name: String,
    prompt: String,
) -> Result<(), String>;
...
// 删除模板
#[tauri::command]
async fn delete_article_prompt_template(id: i64) -> Result<(), String>;

// 设置默认模板
#[tauri::command]
async fn set_default_article_prompt_template(id: i64) -> Result<(), String>;
...
| 组件 | 说明 |
|------|------|
| ArticleGenerationPanel | 文章生成主面板 |
| ArticleStyleSelector | 风格选择器 |
| ArticleWordCountSelector | 字数选择器 |
| ArticlePromptSelector | Prompt 模板选择器 |
| ArticlePromptEditor | 模板编辑器（弹窗） |
| MarkdownEditor | Markdown 编辑器 |
| ExportButton | 导出按钮 |
| PromptTemplateSettings | 设置中心 - Prompt 模板标签页 |

### 4.9.9 PDF 导出技术方案

| 方案 | 库 | 优点 | 缺点 |
|------|-----|------|------|
| Rust 原生 | pdf_oxide | 纯 Rust，无外部依赖 | 功能较少 |
| Rust 原生 | fullbleed | HTML/CSS 转 PDF，功能丰富 | 需要 HTML 模板 |

**推荐方案**：fullbleed（HTML/CSS 转 PDF）
