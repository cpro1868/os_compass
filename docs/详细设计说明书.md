...
**FeaturePlugin vs SourcePlugin 的本质区别**：

| 维度 | SourcePlugin（平台插件） | FeaturePlugin（功能插件） |
|------|------------------------|----------------------|
| 入口 | 项目导入时自动触发 | 用户主动触发或定时执行 |
| 生命周期 | 一次性获取数据 | 可能需要持续运行 |
| 数据存储 | 获取数据存入 SQLite | 处理结果可能推送到外部 |
| 配置 | Token 等认证信息 | 功能参数、订阅源等 |
