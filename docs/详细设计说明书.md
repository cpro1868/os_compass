...
impl PluginManager {
    ...
    pub async fn init_all(&self) {
        for plugin in &self.registry.plugins {
            let config = self.load_plugin_config(plugin.id());
            let context = self.build_context(plugin.id(), config);
            plugin.init(&context).await.ok();
        }
    }

    /// 动态启用插件（立即生效）
    pub async fn enable(&self, plugin_id: &str) -> Result<(), PluginError> {
        let plugin = self.find(plugin_id)?;
        let config = self.load_plugin_config(plugin_id);
        let context = self.build_context(plugin_id, config);
        plugin.on_enable(&context).await?;
    ...
    pub async fn disable(&self, plugin_id: &str) -> Result<(), PluginError> {
        let plugin = self.find(plugin_id)?;
        let context = self.build_context(plugin_id, self.load_plugin_config(plugin_id));
        plugin.on_disable(&context).await?;
