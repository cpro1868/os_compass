...
用户输入业务需求描述，系统通过三层优先级搜索机制，从本地库、信息源缓存和联网搜索中推荐相关开源项目。

### 10.2 使用场景

- 用户有业务需求，不知道用什么开源项目
- 用户想探索某个领域有哪些优秀项目
- 用户想从自己关注的信息源中快速检索相关项目

### 10.3 三层搜索优先级

| 优先级 | 搜索范围 | 说明 | 延迟 |
|--------|----------|------|------|
| 第一层 | 本地项目库 | 在已入库的项目中检索，基于关键词匹配 | 极低（本地查询） |
| 第二层 | 信息源缓存 | 在用户配置的信息源已拉取缓存内容中检索 | 低（本地查询） |
| 第三层 | 联网搜索 | 调用外部搜索 API 补充结果（fallback） | 中（网络请求） |

> **架构预留**：三层架构预留 RAG 向量检索扩展点，未来可升级为"本地向量库优先 + 联网降级"的双轨制。

### 10.4 功能设计

| 功能 | 说明 |
|------|------|
| 自然语言输入 | 用户输入业务需求描述（ChatGPT 风格对话界面） |
| LLM 关键词提取 | LLM 分析需求，提取技术关键词和搜索策略 |
| 三层搜索 | 按优先级依次搜索：本地库 → 信息源缓存 → 联网搜索 |
| 结果展示 | 返回匹配项目列表，含匹配度、来源、描述，支持多轮对话 |
| 一键导入 | 搜索结果可一键保存到本地项目库（自动调用项目导入流程） |
| 搜索历史 | 搜索历史持久化存储，右侧边栏显示历史记录 |
| 信息源管理 | 用户独立配置意图搜索的信息源（与雷达信息源独立） |

### 10.5 联网搜索策略

用户可在设置中选择联网搜索策略（第三层 fallback）：

| 策略 | 说明 | 需要 |
|------|------|------|
| GitHub/Gitee Search API | 用平台 Search API 搜索仓库，精准免费 | 平台 Token |
| Tavily/Perplexity | 专为 LLM 设计的全网搜索 API | 搜索 API Key |
| web2md 爬虫 | 爬取搜索引擎结果页，解析搜索结果 | web2md 服务 |
| 禁用 | 不使用联网搜索，仅搜索本地库和缓存 | 无 |

### 10.6 信息源类型

意图搜索独立管理信息源，与情报雷达互不干扰。支持的信息源类型：

| 类型 | 说明 | 技术 |
|------|------|------|
| RSS/Atom | 标准 RSS/Atom feed URL | feed-rs 解析 |
| 网站爬取 | 指定网站 URL，爬取页面内容 | web2md 爬虫服务 |
| Telegram 频道 | 爬取 TG 频道网页版 | web2md 爬取 t.me/s/频道名 |
| 平台预设源 | GitHub/Gitee Trending 等内置 RSS | 预置 RSS URL |
...
| 信息源配置 | 插件独立库 `plugin_search.db` | search_sources 表 |
| 内容缓存 | 插件独立库 `plugin_search.db` | search_cache 表 |
| 搜索历史 | 插件独立库 `plugin_search.db` | search_history 表 |

> **插件数据库策略**：系统提供独立 .db 文件 API 和主库读写 API，插件开发者自行决定数据存储方式。意图搜索插件默认使用独立 .db。
...
```mermaid
flowchart TD
    A[定时拉取信息源] --> B[LLM 解析提取项目信息]
    B --> C[URL 去重]
    C --> D{已存在?}
    D -->|是| E[合并来源列表]
    D -->|否| F[存入 radar_items]
    F --> G[系统通知 + 应用内徽标]
    G --> H[用户查看收件箱]
    H --> I{用户操作}
    I -->|导入| J[调用项目导入流程 入库]
...
| 策略 | 说明 |
|------|------|
| 独立 .db 文件 | 系统提供 `plugin_get_db_path(plugin_id)` API，返回插件专属 .db 文件路径 |
| 主库读写 | 插件可直接读写主库 `os_compass.db`（如将搜索结果导入 projects 表） |
| 开发者决定 | 插件开发者自行决定使用独立 .db、主库、或两者结合 |
| 仓库隔离 | 插件 .db 文件跟随仓库目录（与 os_compass.db 同级），切换仓库时加载对应数据 |
