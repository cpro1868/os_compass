...
|------|----------|----------|-----------|----------|------|
| GitHub 插件 | REST API | `api.github.com` | 可选 | Commit + Issue + Release | ✅ 已实现 |
| Gitee 插件 | Open API | `gitee.com/api/v5` | 可选 | 类似 GitHub | ✅ 已实现 |
| NPM 插件 | Registry API | `registry.npmjs.org` | 不需要 | 下载量 + 版本 + 依赖健康 | ⬜ 待开发 |
| PyPI 插件 | JSON API | `pypi.org/pypi/{pkg}/json` | 不需要 | 发布活跃度 + Python兼容性 | ⬜ 待开发 |
| Crates.io 插件 | REST API | `crates.io/api/v1` | 不需要 | 下载量 + Rust版本支持 | ⬜ 待开发 |
| Crawler 插件 | 爬虫服务 | HTTP API | 不需要 | 无评分（标记为"未评分"） | ✅ 已实现 |

### 4.3 平台扩展详细分析

> **参照文档**：`docs/M19平台扩展详细分析.md`
...
| 评价指标 | 下载量30% + 维护状态20% + 依赖健康15% + 维护者15% + 许可证10% + GitHub关联10% |
...
| 评价指标 | 发布活跃度25% + Python兼容性20% + 分类完善度15% + 文档完整性15% + 许可证15% + 维护状态10% |
...
| 评价指标 | 下载量25% + 近期活跃度20% + Rust版本支持20% + 分类完善度15% + 文档完整性10% + 许可证10% |
...
struct HealthScore {
    total: f32,           // 总分 0-100
    grade: ScoreGrade,      // Excellent/Good/Fair/Poor/Bad
    breakdown: Vec<ScoreItem>,  // 分项得分
}
...
> **功能定位**：通过用户参与式标记 + LLM 智能分析，构建自适应广告屏蔽系统。
> **更新日期**：2026-08-17
...
#### 11.10.2 与 11.9 广告过滤插件的关系

| 维度 | 广告过滤插件（banlex） | 广告学习功能（LLM） |
|------|------------------------|---------------------|
| 原理 | 词库匹配 | LLM 语义分析 |
| 主动性 | 被动过滤 | 用户主动标记 |
| 智能化 | 固定规则 | 自动学习 |
...
**两者关系**：互补使用。banlex 过滤通用垃圾，广告学习处理个性化/新型广告。
...
|------|------|
| 标记广告 | 雷达卡片增加"标记广告"按钮 |
| LLM 判定 | 分析标题+摘要，提取广告关键词/域名/正则 |
| 自动学习 | 置信度达到阈值自动加入屏蔽列表 |
| 白名单 | 误判可加入白名单 |
| 命中统计 | 记录屏蔽规则命中次数 |
| 自适应调整 | 连续命中自动提升置信度 |
...
| 表名 | 位置 | 用途 |
|------|------|------|
| ad_patterns | 系统库 | 屏蔽规则 |
| ad_whitelist | 系统库 | 误判白名单 |
| ad_feedback | 系统库 | 用户反馈记录 |
