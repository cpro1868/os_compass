...
使用 sqlite-vss 创建向量虚拟表，支持语义相似度搜索。
...
> **重要**：仓库数据库（`os_compass.db`）初始化时必须加载 sqlite-vss 扩展并创建向量表。
...
| 组件 | 位置 | 说明 |
|------|------|------|
| sqlite-vss 扩展 | 系统库目录 | 全局共享，所有仓库共用一个扩展 |
...
| 平台 | 扩展文件 | 加载方式 |
|------|----------|----------|
| Windows | `vss0.dll` | `PRAGMA load_extension('vss0.dll');` |
| macOS | `vss0.dylib` | `PRAGMA load_extension('vss0.dylib');` |
| Linux | `vss0.so` | `PRAGMA load_extension('vss0.so');` |

**扩展打包**：

sqlite-vss 扩展需要随应用打包：
