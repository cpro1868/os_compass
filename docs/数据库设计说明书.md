...
| 版本 | V1.4 |
| 修订日期 | 2026-07-11 |
| 修订说明 | 新增二期 M13 表结构（feature_plugins、plugin_radar.db、plugin_search.db） |
...
## 4. 二期扩展表（M13：系统插件 + 意图搜索 + 情报雷达）

> **二期功能标记**：以下所有表均为二期（Phase 2）设计，不在 MVP 范围内。

### 4.1 主库扩展表

#### 4.1.1 feature_plugins（功能插件注册表）
...
预设数据：
```sql
INSERT OR IGNORE INTO feature_plugins (id, name, plugin_type, enabled, version) VALUES
('radar', '情报雷达', 'radar', 0, '1.0.0'),
('search', '意图搜索', 'search', 0, '1.0.0');
```

### 4.2 插件独立库

> **插件数据库策略**：系统提供独立 .db 文件 API 和主库读写 API，插件开发者自行决定。以下为两个内置插件的默认独立库设计。

#### 4.2.1 plugin_radar.db（情报雷达插件库）

存放位置：仓库目录下 `plugin_radar.db`（与 `os_compass.db` 同级）

##### 4.2.1.1 radar_sources（信息源表）
...
#### 4.2.2 plugin_search.db（意图搜索插件库）

存放位置：仓库目录下 `plugin_search.db`（与 `os_compass.db` 同级）

##### 4.2.2.1 search_sources（搜索信息源表）

结构与 radar_sources 相同，但独立管理。
