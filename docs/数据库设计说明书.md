...
### 3.2 project_embeddings（项目向量表）

> **新增日期**：2026-07-27
> **用途**：存储项目 embedding 向量，用于意图搜索的本地语义匹配
> **技术**：sqlite-vec 扩展
> **存储位置**：仓库库 `os_compass.db`（跟随仓库切换）

使用 sqlite-vec 创建向量虚拟表，支持语义相似度搜索。
...
> **初始化说明**：向量表在仓库创建（create_vault）时自动创建，详见 §7.9。

**字段说明**：

| 字段 | 类型 | 说明 |
|------|------|------|
| project_id | INTEGER | 项目 ID，与 projects 表关联 |
| embedding | BLOB(1536) | 1536 维向量（二进制存储） |

**向量生成策略**：

| 时机 | 说明 |
|------|------|
| 项目导入时 | 自动生成（基于 summary + description） |
| AI 分析后 | 重新生成（summary 可能更新） |
| 手动触发 | 用户可触发重建 |

**向量内容**：
- 优先使用 `summary` 字段（一句话总结）
- 如果没有 summary，使用 `description` 字段
- 如果都没有，使用项目 `name`
...
```sql
-- 生成查询向量
SELECT vss_search_params('query embedding here', 5);

-- 语义搜索
SELECT p.*, distance
FROM project_embeddings e
JOIN projects p ON e.project_id = p.id
JOIN vss_search('project_embeddings', 'query embedding') v ON e.rowid = v.rowid
ORDER BY v.distance
LIMIT 10;
```
...
> **重要**：仓库数据库（`os_compass.db`）初始化时必须加载 sqlite-vec 扩展并创建向量表。
...
1. 创建数据库文件
2. 加载 sqlite-vec 扩展
3. 创建业务表（projects, categories, tags 等）
4. 创建向量虚拟表（project_embeddings）
```
...
```sql
-- 1. 加载 sqlite-vec 扩展
-- 注意：Windows 上使用 .dll，macOS/Linux 上使用 .so
PRAGMA load_extension('vss');
...
| 项目 | 说明 |
|------|------|
| 表类型 | SQLite 虚拟表（sqlite-vec 扩展） |
| 存储位置 | 仓库库 `os_compass.db`（**与 projects 表同一文件**） |
| 向量数据 | 每个仓库独立的向量数据 |
| 向量维度 | 1536（OpenAI text-embedding-3-small 默认） |
| 关联字段 | project_id（与 projects 表 JOIN） |
| 用途 | 意图搜索本地向量语义匹配 |

> 向量数据和项目数据在同一文件，可直接 JOIN 查询。

**⚠️ 技术选型变更（2026-08-02）**：
> 原方案 sqlite-vss **不支持 Windows**，已迁移至 **sqlite-vec**。

**扩展说明**：

| 组件 | 位置 | 说明 |
|------|------|------|
| sqlite-vec 扩展 | 系统库目录 | 全局共享，所有仓库共用一个扩展 |
| 向量数据表 | 仓库库 | 每个仓库独立的向量数据 |

**平台适配**：

| 平台 | 扩展文件 | 加载方式 |
|------|----------|----------|
| Windows | `vec0.dll` | `PRAGMA load_extension('vec0.dll');` |
| macOS | `vec0.dylib` | `PRAGMA load_extension('vec0.dylib');` |
| Linux | `vec0.so` | `PRAGMA load_extension('vec0.so');` |

**扩展打包**：

sqlite-vec 扩展需要随应用打包：
- Tauri：在 `src-tauri/capabilities` 或构建脚本中包含扩展
- 扩展文件放在应用 resources 目录
- 运行时通过 `PRAGMA load_extension()` 加载
...
> **更新日期**：2026-08-14
> **更新说明**：
> - 删除 search_cache 表，改为本地向量搜索
> - **M22 更新**：search_history 表从仓库库移至**系统库** `plugins.db`

存放位置：仓库目录下 `plugin_search.db`（与 `os_compass.db` 同级）

##### 9.2.2.1 search_sources（搜索信息源表）

> **功能调整**：意图搜索不再依赖信息源缓存，改为直接使用本地向量搜索 + LLM 联网搜索。
> 搜索信息源配置保留，用于未来可能的扩展场景（如用户自定义 LLM 搜索策略）。
...
##### 9.2.2.2 search_history（搜索历史表）→ M22 移至系统库

> **M22 更新**：搜索历史表从仓库库移至**系统库** `plugins.db`，因为搜索历史是用户行为记录，与具体仓库无关，方便跨仓库使用。
...
> **注意**：M14/M15 的备份恢复、新手引导、快捷键功能**不需要新增数据库表**。
> - 备份恢复：文件级别操作，直接操作 .zip 文件，无需数据库记录
> - 快捷键：存储在现有 `app_settings` 表的 JSON 字段中
> - 新手引导：使用 `localStorage` 记录完成状态

### 10.1 快捷键配置（存储在 app_settings 表）

> 快捷键配置作为用户偏好，存储在 `app_settings.settings` JSON 字段中。

| 字段 | 说明 |
|------|------|
| shortcuts | 用户自定义快捷键配置 JSON |

```json
{
  "shortcuts": [
    {"id": "cmd_palette", "key": "k", "modifiers": ["ctrl"], "action": "OpenCommandPalette", "enabled": true},
    {"id": "new_project", "key": "n", "modifiers": ["ctrl"], "action": "NewProject", "enabled": true},
    {"id": "settings", "key": ",", "modifiers": ["ctrl"], "action": "OpenSettings", "enabled": true},
    {"id": "help", "key": "/", "modifiers": ["ctrl"], "action": "ShowHelp", "enabled": true}
  ]
}
```
...
### 11.1 ad_patterns（广告屏蔽表）

**存储位置**：系统库 `plugins.db`（全局共享）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | |
| pattern_type | TEXT | NOT NULL | 'keyword' / 'domain' / 'regex' |
| pattern_value | TEXT | NOT NULL | 关键词/域名/正则表达式 |
| confidence | INTEGER | DEFAULT 50 | 置信度 1-100 |
| source | TEXT | DEFAULT 'manual' | 'manual' / 'llm_learned' |
| hit_count | INTEGER | DEFAULT 0 | 累计命中次数 |
| consecutive_hits | INTEGER | DEFAULT 0 | 连续命中次数 |
...
CREATE INDEX IF NOT EXISTS idx_ad_patterns_confidence ON ad_patterns(confidence DESC);
CREATE INDEX IF NOT EXISTS idx_ad_patterns_enabled ON ad_patterns(enabled);
```

### 11.2 ad_whitelist（广告白名单）
...
|------|------|------|------|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | |
| url_hash | TEXT | NOT NULL UNIQUE | 来源 URL 哈希 |
...
CREATE INDEX idx_ad_whitelist_hash ON ad_whitelist(url_hash);
```

### 11.3 ad_feedback（广告反馈表）

**用途**：记录用户反馈，用于优化 LLM 和分析广告趋势。

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | |
| item_id | TEXT | | 雷达条目 ID |
| item_title | TEXT | | 条目标题（脱敏存储） |
| item_url_hash | TEXT | | URL 哈希 |
| action | TEXT | NOT NULL | 操作：marked_ad/marked_legit/unmarked |
| llm_judgment | INTEGER | | LLM 判定：1=广告，0=非广告 |
| user_judgment | INTEGER | | 用户反馈：1=广告，0=非广告 |
| pattern_id | INTEGER | | 匹配的规则 ID |
| llm_confidence | INTEGER | | LLM 置信度 |
| created_at | TEXT | DEFAULT datetime('now','localtime') | |
...
CREATE TABLE IF NOT EXISTS ad_feedback (
    ...
    llm_judgment INTEGER,
    user_judgment INTEGER,
    pattern_id INTEGER REFERENCES ad_patterns(id),
    llm_confidence INTEGER,
    created_at TEXT DEFAULT (datetime('now', 'localtime'))
);

CREATE INDEX idx_ad_feedback_action ON ad_feedback(action);
CREATE INDEX idx_ad_feedback_pattern ON ad_feedback(pattern_id);
CREATE INDEX idx_ad_feedback_created ON ad_feedback(created_at DESC);
```
...
| mode | TEXT | DEFAULT 'interval' | 'interval'=间隔模式，'custom'=自定义模式 |
| interval_seconds | INTEGER | DEFAULT 1800 | 采集间隔（秒） |
| custom_unit | TEXT | | 'second'/'minute'/'hour' |
| custom_value | INTEGER | | 自定义数值 |
| notification_enabled | INTEGER | DEFAULT 1 | 0=关闭，1=开启 |
| system_notification | INTEGER | DEFAULT 1 | 系统通知 |
| badge_notification | INTEGER | DEFAULT 1 | 雷达图标徽章 |
| last_run_at | TEXT | | 上次执行时间 |
| next_run_at | TEXT | | 下次执行时间 |
| new_items_count | INTEGER | DEFAULT 0 | 新增条目数 |
| updated_at | TEXT | DEFAULT datetime('now','localtime') | |
...
### 12.2 radar_notifications（通知记录表）
...
| body | TEXT | | 通知内容 |
| item_count | INTEGER | DEFAULT 0 | 新增数量 |
...
CREATE INDEX IF NOT EXISTS idx_notifications_read ON radar_notifications(read);
```
...
### 13.1 platform_packages（平台包缓存表）

**存储位置**：仓库库 `os_compass.db`

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | |
| platform | TEXT | NOT NULL | 'npm' / 'pypi' / 'crates' / 'dockerhub' |
| platform_id | TEXT | NOT NULL | 平台原生 ID（如 npm 包名） |
| name | TEXT | NOT NULL | 显示名称 |
| version | TEXT | NOT NULL | 最新版本 |
| description | TEXT | | 描述 |
| homepage | TEXT | | 官网链接 |
| repository_url | TEXT | | 源码仓库链接 |
| license | TEXT | | 许可证 |
| downloads | INTEGER | | 总下载量 |
| recent_downloads | INTEGER | | 近期下载量 |
| stars | INTEGER | | 星标数 |
| version_count | INTEGER | | 版本数量 |
| last_release | TEXT | | 最后发布时间 |
| supported_versions | TEXT | | 支持的版本范围 |
| health_score | REAL | | 健康度评分 |
| health_grade | TEXT | | Excellent/Good/Fair/Poor/Bad |
...
| V1.4 | 2026-07-20 | 新增 M14/M15 说明（备份恢复无需表、快捷键存 app_settings、引导用 localStorage） |
| V1.5 | 2026-07-27 | M13 意图搜索：删除 search_cache 表，改为关键词匹配 |
| V1.6 | 2026-07-27 | 新增 M17 向量搜索：project_embeddings 向量表 + §7.9 向量扩展初始化 |
