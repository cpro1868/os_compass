...
| M15 主页增强 + 用户信息 + 雷达增强 | ⚠️ 需重构 | 2026-07-22 |

> **M15 问题**：广告过滤功能未正确集成 banlex，交付质量差，需要重构。
...
| 阶段 | 状态 | 说明 |
|------|------|------|
| M15 重构 | 🚨 进行中 | 广告过滤需要正确集成 banlex |
| M16 主页引导 | 📋 规划中 | 详见 M16 实施计划 |
...
| Task 2 | 用户信息 | 数据保险箱前端 UI | ✅ |
| Task 3 | 雷达增强 | 搜索增强后端 API | ✅ |
| Task 4 | 雷达增强 | 搜索增强前端 UI | ✅ |
| Task 5 | 广告过滤 | banlex 集成 + 中文词库 | 🚨 **需重构** |
| Task 6 | 广告过滤 | 过滤设置 UI | ✅ |
| Task 7 | 广告过滤 | 过滤流程集成 | 🚨 **需重构** |
...
| 问题 | 说明 |
|------|------|
| banlex 未集成 | 需要从 GitHub 添加依赖 |
| 词库质量差 | 需要创建 JSON 格式词库 |
