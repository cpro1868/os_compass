...
### 原型
`design-system/public/home.html`
...
| # | 功能 | 说明 | 状态 |
|---|------|------|------|
| 1 | 意图搜索 | 聊天模式搜索项目 | 🚧 需完善后端 |
| 2 | 搜索结果展示 | 结果卡片（名称、星标、活跃度） | 🚧 待实现 |
| 3 | 快捷提示 | 搜索框上方快捷按钮 | 🚧 待实现 |
| 4 | 最近使用侧栏 | 右侧固定栏，竖向排列 | 🚧 待实现 |
| 5 | 项目详情跳转 | 点击项目跳转详情页 | 🚧 待实现 |
| 6 | 首页入口 | 侧边栏 Logo 下方 | ✅ 已完成 |
| 7 | 默认启动页 | App 启动默认显示首页 | ✅ 已完成 |
| 8 | 系统托盘 | 图标 + 菜单（显示/隐藏、退出） | ✅ 已完成 |
| 9 | 最小化到托盘 | 关闭按钮 → 托盘 | ✅ 已完成 |
| 10 | 主题适配 | 跟随 App 浅色/深色 | ✅ 已完成 |

### 意图搜索待完善
| # | 问题 | 修复方案 |
|---|------|----------|
| 1 | 搜索质量差 | 分词 + 权重查询 |
| 2 | 缺少项目信息 | JOIN projects 表 |
| 3 | 评分不合理 | 计算活跃度 |
| 4 | 缓存未填充 | 导入时同步 |
