...
## 2026-09-08 15:44 - 情报雷达收藏页取消默认时间筛选与双向状态隔离 ✅ 已完成

### 需求
用户反馈：情报雷达收藏页不应按默认时间（最近 7 天）筛选，而应展示所有历史收藏；且从收藏页切回全部/未读时，原有的默认时间限制（最近一周）不能受到影响。

### 实施
- `RadarInbox.tsx`：
  - 维护 `generalDateRangeRef` 与 `starredDateRangeRef`，实现常规流（全部/未读）与收藏夹（collected）的日期状态双向隔离与记忆；
  - 切换至收藏 Tab 时，日期输入框自动清空为无限制，全量展示该仓库所有已收藏项目；
  - 切回全部/未读 Tab 时，严格自动恢复其原有的起止日期限制（默认最近一周或切出前的设定）；
  - 收藏页重置保持日期为空，常规页重置恢复最近一周；保留用户在收藏页主动指定日期的过滤能力。
- 验证：
  - 新增 `RadarInbox.starred-date.test.tsx` 2 个自动化测试，全量 12 文件 78 tests 全绿；
  - `pnpm typecheck` 通过；`cargo check --lib --tests` 通过；
  - `pnpm tauri build` MSI + NSIS 成功；
  - 产物更新至 `Previous/os-compass.exe`（SHA-256：`090353FB4A34B00EF2F0FCFB478A9A1D9C2EA9E50A89546761BB13E2089140AA`）。

---

## ✅ 工作包 A：首页搜索与意图搜索统一及体验优化（全部验收完成）

- **验收时间**：2026-09-08 13:02
- **状态**：✅ **已完成 (done)**
- **交付范围**：
  1. 意图搜索（SearchView）与首页搜索功能对齐与全面贯通；
  2. 搜索质量与相关性优化：`SEMANTIC_SEARCH_MIN_SCORE = 0.45` 阈值过滤，低于阈值丢弃，维度校验跳过；
  3. "让大模型再推荐 5 个"重构：新开独立聊天记录直接输出大模型原文；
  4. 原文样式接入 `MarkdownRenderer`（支持加粗/列表/链接排版与亮暗主题）；
  5. 意图分析多轮上下文记忆与智能收敛：避免前言不搭后语与机械无休止追问；
  6. 历史记录单项删除、卡片精简、本地项目跳转详情、外部项目系统浏览器打开；
  7. 76 个前端自动化测试全部通过，MSI + NSIS 安装包构建成功，`Previous/` 已同步最新可运行版本。
...
根据 `docs/当前开发计划.md`：
- **工作包 B**：M23 安装打包与版本发布（自动化构建脚本规范与 release 归档）
- **工作包 C**：向量化时机优化（新增项目时自动/按需向量化）
