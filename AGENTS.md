...
### 规则 2：情报雷达数据存储规则（需求文档明确）

**⚠️ 重要 Tip：情报雷达的数据存储遵循以下规则**
...
| 操作 | 数据位置 | 说明 |
|------|---------|------|
| 信息源 CRUD | 系统库 `plugins.db` | 全局共享，统一管理 |
| 采集数据写入 | 当前仓库 `plugin_radar.db` | 跟随仓库切换 |
| 采集数据读取 | 当前仓库 `plugin_radar.db` | 切换仓库时加载对应仓库数据 |
...
| `docs/M13实施计划.md` | M13 二期实施计划（13 Task） | 实施进度变化时 |
