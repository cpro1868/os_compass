...
**解读**：
1. **查档求证**：不确定的 API 行为先查文档、源码确认
2. **对齐需求**：模糊需求必须对齐后再开工
3. **请示规则**：不确定的业务规则向用户/负责人确认
4. **复用存量**：优先复用现有组件，减少冗余代码
5. **完备校验**：输入校验、边界处理必须完善
6. **恪守规范**：遵循项目架构和代码规范
7. **坦诚存疑**：不懂的地方坦诚承认，不乱猜
8. **分步迭代**：分步骤实现，每步可验证
...
**在持有 Mutex 锁的代码块内，禁止调用内部也获取同一把锁的函数！**

Rust 的 Mutex 不可重入，同一线程两次获取同一把锁会死锁。

详见 `docs/系统关键错误记录.md` #005
