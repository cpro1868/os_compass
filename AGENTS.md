...
### 规则 2：情报雷达数据存储规则（简化设计）

**情报雷达数据统一放在系统库**：
- 信息源（radar_sources）→ 系统库 `plugins.db`
- 采集数据（radar_items）→ 系统目录 `$env:APPDATA\.os-compass\plugin_radar.db`
- 切换仓库时数据保持不变

**注意**：简化设计，所有雷达数据放在系统目录，跟随仓库切换
...
| `docs/M15实施计划.md` | M15 主页增强实施计划（7 Task） | 实施进度变化时 |
